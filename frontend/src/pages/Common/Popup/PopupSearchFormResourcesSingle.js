...
import { Grid, Box, Button, ListItem, CardContent, Collapse, FormGroup, FormControlLabel, Radio, Tabs, Tab } from '@mui/material';
                        ...
                        <Grid container sx={{ pl: 1, pr: 1 }}>
                            {
                                _.map(obj.children, (subObj, subI) => (
                                    <Grid item sm={4} key={i + "_" + subI}>
                                        <FormGroup>
                                            <FormControlLabel control={<Radio id={subObj.key} checked={selectItem.key === subObj.key} onChange={(e) => handleCheckboxChange(subObj)} />} label={subObj.name} />
                                        </FormGroup>
                                    </Grid>
                                ))
                            }
...
const PopupSearchFormResourcesSingle = () => {
    ...
    const handleCheckboxChange = (target) => {
        setSelectResourceObj({ name: target.name, key: target.key, tab: staticMenuKey[menuKey] });
    }
...
export default PopupSearchFormResourcesSingle;